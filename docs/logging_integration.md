...
data feed should be useful in order to gain insights into climber usage
...
from the API. These data loggers are the following:
 ...
 - `awx.analytics.activity_stream` - Record of changes to the objects within the climber app
 - `awx.analytics.system_tracking` - Data gathered by Ansible scan modules ran by scan job templates

These loggers only use log-level of `INFO`. Additionally, the standard climber logs are deliverable through this
...
from the standard climber logs.
...
up, also refer to instructions in [`/tools/elastic/README.md`](github.com/ctrliq/climber/blob/main/tools/elastic/README.md).
...
| Field | Information |
|-----------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `cluster_host_id` | (string) Unique identifier of the host within the climber cluster |
| `level` | (choice of `DEBUG`, `INFO`, `WARNING`, `ERROR`, etc.) Standard python log level, roughly reflecting the significance of the event; all of the data loggers (as a part of this feature) use `INFO` level, but the other climber logs will use different levels as appropriate |
...
| changes           | (string) Unique identifier of the host within the climber cluster                                                         |
...
settings, as documented, 2) taking some action in climber that causes a log
...
**climber logs:** Formatting of Traceback message is a known issue in several
...
not cause climber operations to hang.
