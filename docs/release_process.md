# Releasing climber (and awx-operator)

The release process for climber is mostly automated as of version 19.5.0.
...
1. Open the main project page for [climber](github.com/ctrliq/climber/releases) and [AWX Operator](https://github.com/ansible/awx-operator/releases).

Find the latest releases of the projects on the right hand side of the screen:

![Latest Release](img/latest-release.png)

2. Open the compare screen for the two projects [climber](github.com/ctrliq/climber/compare) and [AWX Operator](https://github.com/ansible/awx-operator/compare).
...
The page will now automatically update with a list of PRs that are in `climber/devel` but not in the last release.
...
If the latest release of `climber` is 19.5.0:
...
To stage the release, maintainers of this repository can run the [Stage Release](github.com/ctrliq/climber/actions/workflows/stage.yml) workflow.
...
![Staging climber](img/stage-release.png)
...
### Releasing climber, awxkit and awx.awx collection

Once staging is complete we can complete the release of awx and the operator.

1. navigate to the [Releases page](github.com/ctrliq/climber/releases) for climber and verify things look ok.
...
4. Add in a message of what operator version is release with this climber version (if applicable):
```
## AWX Operator
...
Once the release is published, another workflow called [Promote Release](github.com/ctrliq/climber/actions/workflows/promote.yml) will start running:
...
![Verify released climber image](img/verify-released-awx-image.png)
...
Once the climber image is live, we can now release the AWX Operator.

1. Navigate to the [Releases page](https://github.com/ansible/awx-operator/releases) for AWX Operator and follow the same process used for publishing the climber draft except note the version of climber released with Operator.
...
Operator hub PRs are generated via an Ansible Playbook. See someone on the climber team for the location of the playbooks and instructions on how to run them.
...
Decide whether or not you can just fall-forward with a new climber Release to fix a bad release. If you need to remove published artifacts from publicly facing repositories, follow the steps below.

Here are the steps needed to revert an climber and an AWX-Operator release. Depending on your use case, follow the steps for reverting just an climber release, an Operator release or both.


1. Navigate to the [climber Release Page](github.com/ctrliq/climber/releases) and delete the climber Release that needs to be removed.

![Revert-1-Image](img/revert-1.png)

2. Navigate to the [climber Tags Page](github.com/ctrliq/climber/tags) and delete the climber Tag that got created by the Github Actions Workflow from when you originally tried to release climber. You need delete the release in step 1 before you can do this step. The tag must not be tied to a release if you want to delete a tag.
...
4. Navigate to [quay.io](https://quay.io/repository/ansible/awx?tag=latest&tab=tags) and delete the published climber image(s) and tags.
...
6. Navigate to the [Ansible Galaxy Collections](https://galaxy.ansible.com/awx/awx) website and remove the published climber collection with the bad tag.

7. Navigate to the [PyPi](https://pypi.org/project/awxkit/#history) and delete the bad climber tag and release that got published.
